...
const deployScaffoldRecipeNFTContract: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const mintNFTContract = await hre.ethers.getContract<Contract>("ScaffoldRecipeNFT", deployer);
  console.log("🚀 Deploy ScaffoldRecipeNFT Contract:", await mintNFTContract.getAddress());
};
