...
const deployMintNFTBaseUriContract: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const mintNFTBaseUriContract = await hre.ethers.getContract<Contract>("MintNFTBaseUri", deployer);
  console.log("🚀 Deploy MintNFTBaseUri Contract:", await mintNFTBaseUriContract.getAddress());
};
